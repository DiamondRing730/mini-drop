...
urllib (no new build dependency, no SDK). It drives the exact same read-only tools as the
Claude path and ends with submit_attribution; the verifier re-checks every number the same
way regardless of which backend produced the finding.
...
def _openai_tools() -> list[dict]:
    """Convert the Anthropic-shaped TOOL_DEFS to OpenAI/DeepSeek function-calling shape."""
    return [
        {
            "type": "function",
            "function": {
...
def run_deepseek(prof: Profile) -> dict | None:
    """Real DeepSeek tool-use loop. Returns None if no key / network / structured result."""
    ...
    try:
        for _ in range(MAX_TOOL_ITERATIONS):
            ...
            if not tool_calls:
                return None  # model answered in prose without submitting — no structured result
