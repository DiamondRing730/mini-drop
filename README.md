...
正式设计文档见 [docs/Mini-Drop设计文档.docx](docs/Mini-Drop设计文档.docx)，覆盖架构、状态机、
关键决策、取舍、AI 协作、性能自证与未来 7 天计划。
...
- 运行中任务禁止删除；单任务异常不会杀死 worker，Agent 重启后会自动接管未完成任务。
...
- 46 条单元测试用例、3 条端到端测试用例；最近完整单测覆盖率约 80%。归因基准见
...
当前进入交付收尾阶段：干净环境复现和演示视频。自然语言采集仍为未实现
的可选加分项。
